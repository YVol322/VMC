...
int main() {    
    ...
    unsigned int numberOfMetropolisSteps = (unsigned int) 5e3;
	...
	std::vector<double> stepLength_arr = {1e-1, 1e-2};
	std::vector<double> learning_rate_arr = {1e-1, 1e-2};
	double stop_at = 0.01;
	double max_iters = 50;
	double iter = 0;
	double l2_norm = 4.1;
    ...
    outFile << "Iter\tEta\tH\tEnergy\tgrad" << endl;
    ...
    for(double stepLength: stepLength_arr)
    {
        for(double learning_rate: learning_rate_arr)
        {
            ...
            outFile << iter << "\t" << learning_rate << "\t" << stepLength << "\t" 
                << total_El / n_threads << "\t" << l2_norm << std::endl;
