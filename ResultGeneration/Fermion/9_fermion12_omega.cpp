...
int main() {    
	...
	std::vector<double> learning_rate_arr = {1e-2};
	double stop_at = 0.01;
	double max_iters = 15;
	double iter = 0;
	double l2_norm = 4.1;
	...
	std::vector<double> betaPJ0(1, 0.385);
    std::vector<double> grad_beta(numberOfPairs, 1);
    std::vector<double> grad_betaPJ(1, 1);

    std::string path = "Results/Tables/Fermions/Interacting/Program9";
    ...
    for(double stepLength: stepLength_arr)
    {
        for(double learning_rate: learning_rate_arr)
        {
            ...
            while(iter < max_iters && l2_norm > stop_at)
            {
                ...
                outFile << iter << "\t" << total_El / n_threads << "\t" << l2_norm << "\t" << betaPJ[0] << endl;
            }
