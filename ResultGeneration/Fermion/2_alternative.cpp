...
int main() {    
    ...
    std::string path = "Results/Tables/Fermions/Interacting/Program2";
    ...
    std::vector<double> beta0(numberOfPairs, 0.341);
	std::vector<double> betaPJ0(1, 0.375);
    ...
    while(iter < max_iters && l2_norm > stop_at)
    {
        ...
        outFile << iter << "\t" << total_El / n_threads << "\t" << l2_norm << "\t" << betaPJ[0] << std::endl;
    }

    total_El = 0.0;
    numberOfMetropolisSteps = (unsigned int) 1e7;
    numberOfEquilibrationSteps = (unsigned int) 1e5;
