...
int main() {    
    ...
    unsigned int numberOfMetropolisSteps = (unsigned int) 5e4;
    unsigned int numberOfEquilibrationSteps = (unsigned int) 5e3;
	...
	std::vector<double> stepLength_arr = {1e-1};
    ...
    std::vector<double> beta0(numberOfPairs, 0.41);
	std::vector<double> betaPJ0(1, 0.341);
    std::vector<double> grad_beta(numberOfPairs, 1);
    std::vector<double> grad_betaPJ(1, 1);

    std::string path = "Results/Tables/Fermions/Interacting/Program4";
    std::filesystem::create_directories(path);


    std::string filename2 = path
        + "/11beta_iter_PJ_MH.cvv";
    ...
    for(double stepLength: stepLength_arr)
    {
        for(double learning_rate: learning_rate_arr)
        {
            ...
            while(iter < max_iters && l2_norm > stop_at)
            {
                ...
                outFile << iter << "\t" << total_El / n_threads << "\t" << l2_norm << std::endl;
                if(mode == 0)
                {
                    outFile2 << iter;
                    for(int l = 0; l < numberOfPairs; l++)
                ...
                {   
                    outFile2 << iter;
                    outFile2 << "\t" << betaPJ[0] << std::endl;
                }
            }

            total_El = 0.0;
            unsigned int numberOfMetropolisSteps = (unsigned int) 5e5;
            unsigned int numberOfEquilibrationSteps = (unsigned int) 5e4;
