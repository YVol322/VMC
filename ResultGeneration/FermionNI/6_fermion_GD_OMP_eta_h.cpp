...
{    
    ...
    std::string path = "Results/Tables/Fermions/NonInteracting/Program6";
    ...
    outFile << "Iter\tEta\tH\tEnergy" << endl;
    ...
    for(double stepLength: stepLength_arr)
    {
        for(double learning_rate: learning_rate_arr)
        {
            ...
            outFile << iter << "\t" << learning_rate << "\t" << stepLength << "\t" << total_energy / n_threads << endl;
        }
    }
    return 0;
}
