...
{    
    ...
    std::string path = "Results/Tables/Bosons/Program5";
    ...
    sss << log10(learning_rate);
    std::string eta_str = sss.str(); 

    std::string filename = path
        + "/"
        + std::to_string(int(numberOfDimensions))
        + "D_N="
        + std::to_string(int(numberOfParticles))
        + "_alpha="
        + a_str
        + "_eta=1e"
        +  eta_str;
    ...
    outFile << "Iter\tEnergy\tGrad\tAlpha" << endl;
    ...
    while(iter < max_iters && l2_norm > stop_at)
    {
        ...
        outFile << iter << "\t" << mean_energy << "\t" << l2_norm << "\t" << alpha << endl;
